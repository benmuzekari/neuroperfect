...
filename = _thisDir + os.sep + u'data/%s_%s_%s' % (expInfo['test'], expName, expInfo['date'])

# An ExperimentHandler isn't essential but helps with data saving
thisExp = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='/Users/benmuzekari/Desktop/neuroperfect_task/run_task_lastrun.py',
...
# Initialize components for Routine "type_email"
type_emailClock = core.Clock()
subject_email = visual.TextBox2(
...
key_resp_email = keyboard.Keyboard()

# Initialize components for Routine "get_email"
get_emailClock = core.Clock()
email_instructions = visual.TextStim(win=win, name='email_instructions',
...
# ------Prepare to start Routine "type_email"-------
continueRoutine = True
# update component parameters for each repeat
key_resp_email.keys = []
key_resp_email.rt = []
_key_resp_email_allKeys = []
# keep track of which components have finished
type_emailComponents = [subject_email, key_resp_email]
for thisComponent in type_emailComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
type_emailClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "type_email"-------
while continueRoutine:
    # get current time
    t = type_emailClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=type_emailClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *subject_email* updates
    if subject_email.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        subject_email.frameNStart = frameN  # exact frame index
        subject_email.tStart = t  # local t and not account for scr refresh
        subject_email.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(subject_email, 'tStartRefresh')  # time at next scr refresh
        subject_email.setAutoDraw(True)
    
    # *key_resp_email* updates
    waitOnFlip = False
    if key_resp_email.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        key_resp_email.frameNStart = frameN  # exact frame index
        key_resp_email.tStart = t  # local t and not account for scr refresh
        key_resp_email.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(key_resp_email, 'tStartRefresh')  # time at next scr refresh
        key_resp_email.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(key_resp_email.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(key_resp_email.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if key_resp_email.status == STARTED and not waitOnFlip:
        theseKeys = key_resp_email.getKeys(keyList=['return'], waitRelease=False)
        _key_resp_email_allKeys.extend(theseKeys)
        if len(_key_resp_email_allKeys):
            key_resp_email.keys = _key_resp_email_allKeys[-1].name  # just the last key pressed
            key_resp_email.rt = _key_resp_email_allKeys[-1].rt
...
# -------Ending Routine "type_email"-------
for thisComponent in type_emailComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('subject_email.text',subject_email.text)
thisExp.addData('subject_email.started', subject_email.tStartRefresh)
thisExp.addData('subject_email.stopped', subject_email.tStopRefresh)
# check responses
if key_resp_email.keys in ['', [], None]:  # No response was made
    key_resp_email.keys = None
thisExp.addData('key_resp_email.keys',key_resp_email.keys)
if key_resp_email.keys != None:  # we had a response
    thisExp.addData('key_resp_email.rt', key_resp_email.rt)
thisExp.addData('key_resp_email.started', key_resp_email.tStartRefresh)
thisExp.addData('key_resp_email.stopped', key_resp_email.tStopRefresh)
thisExp.nextEntry()
# the Routine "type_email" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
# keep track of which components have finished
get_emailComponents = [email_instructions, textbox_email, enter_email, submit_text]
for thisComponent in get_emailComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
get_emailClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "get_email"-------
while continueRoutine:
    # get current time
    t = get_emailClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=get_emailClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *email_instructions* updates
    if email_instructions.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        email_instructions.frameNStart = frameN  # exact frame index
        email_instructions.tStart = t  # local t and not account for scr refresh
        email_instructions.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(email_instructions, 'tStartRefresh')  # time at next scr refresh
        email_instructions.setAutoDraw(True)
    
    # *textbox_email* updates
    if textbox_email.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        textbox_email.frameNStart = frameN  # exact frame index
        textbox_email.tStart = t  # local t and not account for scr refresh
        textbox_email.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(textbox_email, 'tStartRefresh')  # time at next scr refresh
        textbox_email.setAutoDraw(True)
    if textbox_email.status == STARTED:  # only update if drawing
        textbox_email.setPadding(, log=False)
    
    # *enter_email* updates
    waitOnFlip = False
    if enter_email.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        enter_email.frameNStart = frameN  # exact frame index
        enter_email.tStart = t  # local t and not account for scr refresh
        enter_email.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(enter_email, 'tStartRefresh')  # time at next scr refresh
        enter_email.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(enter_email.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(enter_email.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if enter_email.status == STARTED and not waitOnFlip:
        theseKeys = enter_email.getKeys(keyList=['enter', 'return'], waitRelease=False)
        _enter_email_allKeys.extend(theseKeys)
        if len(_enter_email_allKeys):
            enter_email.keys = _enter_email_allKeys[-1].name  # just the last key pressed
            enter_email.rt = _enter_email_allKeys[-1].rt
            # a response ends the routine
            continueRoutine = False
...
for thisComponent in get_emailComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('email_instructions.started', email_instructions.tStartRefresh)
thisExp.addData('email_instructions.stopped', email_instructions.tStopRefresh)
thisExp.addData('textbox_email.text',textbox_email.text)
thisExp.addData('textbox_email.started', textbox_email.tStartRefresh)
thisExp.addData('textbox_email.stopped', textbox_email.tStopRefresh)
# check responses
if enter_email.keys in ['', [], None]:  # No response was made
    enter_email.keys = None
thisExp.addData('enter_email.keys',enter_email.keys)
if enter_email.keys != None:  # we had a response
    thisExp.addData('enter_email.rt', enter_email.rt)
thisExp.addData('enter_email.started', enter_email.tStartRefresh)
thisExp.addData('enter_email.stopped', enter_email.tStopRefresh)
thisExp.nextEntry()
thisExp.addData('submit_text.started', submit_text.tStartRefresh)
thisExp.addData('submit_text.stopped', submit_text.tStopRefresh)
